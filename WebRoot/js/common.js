...
function uploadExcel(fileType, obj){
	showProcess("上传中");
	$(obj).parent().parent().ajaxSubmit({
		type: "post",
		...
		success: function(data){
			closeProcess();
			if(data.path != "" ||data.code == "0"){
				if(fileType == "excel") {
					var appStr = "<div style='float:left;margin-left:10px;' imgfile='"+data.path+"'></div>";
					$(obj).parent().parent().append(appStr);
					if (data.code == "0") {
						msgSuccessReload("导入成功！");
					}  else if(data.code == "1"){
						msgError(data.msg);
					}else if(data.code == "2"){
						daoruMsgError(data.msg);
					}
				} else if(fileType == "file") {

				}
			}else{
				msgError("上传文件中，请稍后重试！");
			}
		...
		error : function(data){
			...
			data =JSON.parse(data.split("<audio")[0]);
			if(data.path != "" ||data.code == "0"){
				if(fileType == "excel") {
					var appStr = "<div style='float:left;margin-left:10px;' imgfile='"+data.path+"'></div>";
					$(obj).parent().parent().append(appStr);
